...
class ApiBenchmark {
    ...
    async measureRoute(route: string, method: string = 'GET', body?: any): Promise<void> {
    ...
    async runLoadTest(routes: any[], duration: number = 30, concurrency: number = 50) {
        consola.info({
    ...
    printResults(): void {
        ...
        routeStats.forEach((stats, route) => {
            ...
            consola.log(`  Success Rate: ${((stats.success / stats.count) * 100).toFixed(2)}%`);
            consola.log(`  Avg Duration: ${(stats.totalDuration / stats.count).toFixed(2)}ms`);
        });
    }
}
...
async function runBenchmark() {
    ...
    const routes = [
        ...
        {
            path: '/api/universe/new', method: 'POST',
            body: {
        ...
        },
        {
            ...
            method: 'POST',
            body: { name: `Test User ${Math.random().toString(36).substring(7)}`, email: `test${Math.random().toString(36).substring(7)}@test.com`, password: 'password123' },
        },
    ];
